...
mapping_file_path = '/tmp/jp_markdown_ch11.json'

try:
    print(f"Python script started for Ch11. Notebook path: {os.path.abspath(notebook_path)}")
    ...
    print(f"Mapping file for Ch11 loaded successfully. Contains {len(jp_mapping)} mappings.")
    ...
    for cell_index, cell in enumerate(notebook_data.get('cells', [])):
        if cell.get('cell_type') == 'markdown':
            ...
            # Target Ch11 placeholders
            is_target_chapter_cell = False
            # Using specific Chapter placeholder prefixes for targeting
            temp_str_for_check = original_source_as_string.upper() # For case-insensitive placeholder prefix check
            ch11_prefixes = ["[JP_MD_CH11_", "[JP_CHECK_TITLE_CH11_"]

            if any(prefix.upper() in temp_str_for_check for prefix in ch11_prefixes):
            ...
            if placeholder_found_in_cell and modified_source_string != original_source_as_string:
                ...
                if cell['source'] != new_source_list:
                    ...
                    # print(f"Replaced content in markdown cell {cell_index} (Targeting Chapter 11).") # Verbose

    if cells_modified_count > 0:
        print(f"Total markdown cells modified for Chapter 11: {cells_modified_count}")
        with open(notebook_path, 'w', encoding='utf-8') as f:
            json.dump(notebook_data, f, ensure_ascii=False, indent=2)
        print(f"Successfully updated notebook with Japanese markdown for Chapter 11: {notebook_path}")
    else:
        print("No placeholders for Chapter 11 found or replaced. Check notebook structure, placeholders, and mapping file.")
        debug_cell_count = 0
        for cell_debug_idx, cell_debug_val in enumerate(notebook_data.get('cells', [])):
            if cell_debug_val.get('cell_type') == 'markdown':
                 ...
                 if any(prefix.upper() in source_str_debug.upper() for prefix in ch11_prefixes):
                    print(f"Debug: Ch11 MD Cell {cell_debug_idx} Source: {source_str_debug[:200]}...")
                    debug_cell_count+=1
            if debug_cell_count > 10 :
                break
        if debug_cell_count == 0:
            print("Debug: No Ch11 markdown cells with expected placeholders found during debug scan.")
