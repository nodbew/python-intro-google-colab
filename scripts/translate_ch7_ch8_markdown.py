...
mapping_file_path = '/tmp/jp_markdown_ch7_ch8.json'

try:
    print(f"Python script started for Ch7-8. Notebook path: {os.path.abspath(notebook_path)}")
    ...
    print(f"Mapping file for Ch7-8 loaded successfully. Contains {len(jp_mapping)} mappings.")
    ...
    for cell_index, cell in enumerate(notebook_data.get('cells', [])):
        if cell.get('cell_type') == 'markdown':
            ...
            # Using specific Chapter placeholder prefixes for targeting
            temp_str_for_check = original_source_as_string.upper()
            ch7_ch8_prefixes = ["[JP_MD_CH7_", "[JP_MD_CH8_", "[JP_CHECK_TITLE_CH7_", "[JP_CHECK_TITLE_CH8_"]

            if any(prefix.upper() in temp_str_for_check for prefix in ch7_ch8_prefixes):
            ...
            if placeholder_found_and_replaced_in_cell and modified_source_string != original_source_as_string:
                ...
                if cell['source'] != new_source_list: # Ensure there's an actual change before incrementing
                    cell['source'] = new_source_list
                    cells_modified_count += 1
                    # print(f"Replaced content in markdown cell {cell_index} (Targeting Chapters 7-8).") # Verbose

    if cells_modified_count > 0:
        print(f"Total markdown cells modified for Chapters 7-8: {cells_modified_count}")
        with open(notebook_path, 'w', encoding='utf-8') as f:
            json.dump(notebook_data, f, ensure_ascii=False, indent=2)
        print(f"Successfully updated notebook with Japanese markdown for Chapters 7-8: {notebook_path}")
    else:
        print("No placeholders for Chapters 7-8 found or replaced. Check notebook structure, placeholders, and mapping file.")
        debug_cell_count = 0
        for cell_debug_idx, cell_debug_val in enumerate(notebook_data.get('cells', [])):
            if cell_debug_val.get('cell_type') == 'markdown':
                 ...
                 if any(prefix.upper() in source_str_debug.upper() for prefix in ch7_ch8_prefixes):
                    print(f"Debug: Ch7/8 MD Cell {cell_debug_idx} Source: {source_str_debug[:200]}...")
                    debug_cell_count+=1
            if debug_cell_count > 20 :
                break
        if debug_cell_count == 0:
            print("Debug: No Ch7/Ch8 markdown cells with expected placeholders found during debug scan.")
