...
mapping_file_path = '/tmp/jp_markdown_ch5_ch6.json'

try:
    print(f"Python script started for Ch5-6. Notebook path: {os.path.abspath(notebook_path)}")
    ...
    print(f"Mapping file for Ch5-6 loaded successfully. Contains {len(jp_mapping)} mappings.")
    ...
    for cell_index, cell in enumerate(notebook_data.get('cells', [])):
        if cell.get('cell_type') == 'markdown':
            ...
            # Check if the cell content is likely part of Chapters 5 or 6
            ...
            if any(prefix in original_source_as_string for prefix in ch5_ch6_prefixes):
            ...
            if is_target_chapter_cell:
                for placeholder, japanese_text in jp_mapping.items():
                    ...
                    if (any(prefix in placeholder for prefix in ch5_ch6_prefixes)) and placeholder in modified_source_string:
    ...
    if cells_modified_count > 0:
        print(f"Total markdown cells modified for Chapters 5-6: {cells_modified_count}")
        ...
        for cell_debug_idx, cell_debug_val in enumerate(notebook_data.get('cells', [])):
            if cell_debug_val.get('cell_type') == 'markdown':
                 ...
                 if any(prefix in source_str_debug for prefix in ch5_ch6_prefixes):
