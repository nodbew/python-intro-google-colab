...
mapping_file_path = '/tmp/jp_markdown_ch9_ch10_partial.json'

try:
    print(f"Python script started for Ch9 & partial Ch10. Notebook path: {os.path.abspath(notebook_path)}")
    ...
    print(f"Mapping file for Ch9 & partial Ch10 loaded successfully. Contains {len(jp_mapping)} mappings.")
    ...
    for cell_index, cell in enumerate(notebook_data.get('cells', [])):
        if cell.get('cell_type') == 'markdown':
            ...
            # Ensure prefixes cover all markdown placeholders including those in check cell titles
            ch9_ch10_prefixes = ["[JP_MD_CH9_", "[JP_MD_CH10_", "[JP_CHECK_TITLE_CH9_", "[JP_CHECK_TITLE_CH10_"]

            if any(prefix.upper() in temp_str_for_check for prefix in ch9_ch10_prefixes):
            ...
            if placeholder_found_and_replaced_in_cell and modified_source_string != original_source_as_string:
                ...
                if cell['source'] != new_source_list:
                    ...
                    # print(f"Replaced content in markdown cell {cell_index} (Targeting Chapters 9-10).") # Verbose

    if cells_modified_count > 0:
        print(f"Total markdown cells modified for Chapters 9-10 (partial): {cells_modified_count}")
        with open(notebook_path, 'w', encoding='utf-8') as f:
            json.dump(notebook_data, f, ensure_ascii=False, indent=2)
        print(f"Successfully updated notebook with Japanese markdown for Chapters 9-10 (partial): {notebook_path}")
    else:
        print("No placeholders for Chapters 9-10 (partial) found or replaced. Check notebook structure, placeholders, and mapping file.")
        debug_cell_count = 0
        for cell_debug_idx, cell_debug_val in enumerate(notebook_data.get('cells', [])):
            if cell_debug_val.get('cell_type') == 'markdown':
                 ...
                 if any(prefix.upper() in source_str_debug.upper() for prefix in ch9_ch10_prefixes):
                    print(f"Debug: Ch9/10 MD Cell {cell_debug_idx} Source: {source_str_debug[:200]}...")
                    debug_cell_count+=1
            if debug_cell_count > 30 :
                break
        if debug_cell_count == 0:
             print("Debug: No Ch9/Ch10 markdown cells with expected placeholders found during debug scan.")
